...
class ScrapingOFAC:
    ...
    def download_ofac_file(self):
        ...
        self.serialize_ofac_list = self.get_serialize_ofac_list()
        self.create_csv()
    ...
    def is_xml_exists(self):
        return os.path.isfile(os.path.join(self.dir, values.OFAC_XMLNAME))
    ...
    def get_serialize_ofac_list(self):
        serialize_ofac_list = []
        for ofac_item in self.ofac_list:
            serialize_ofac = {
                "id": ofac_item.get("uid", "").upper().strip(),
                "firstName": ofac_item.get("firstName", "").upper().strip(),
                "lastName": ofac_item.get("lastName", "").upper().strip(),
                "title": ofac_item.get("title", "").upper().strip(),
                "type": ofac_item.get("sdnType", "").upper().strip(),
                "remarks": ofac_item.get("remarks", "").upper().strip(),
            }
            ...
            if ofac_item.get("programList", False):
                ...
                serialize_ofac["program"] = program_list.upper().strip()
